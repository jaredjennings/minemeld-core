...
def _build_graph(config):
    ...
    # nodes
    for nodename, _ in list(config.nodes.items()):
        graph.add_node(nodename)

    # edges
    for nodename, nodevalue in list(config.nodes.items()):
...
def _plan_subgraph(sg, config, state_info):
    ...
    scheckpoints = sorted(
        [(c, cn.num_sources) for c, cn in list(checkpoints.items()) if c is not None],
        key=itemgetter(1),
        reverse=True
    )
    ...
    init_flag = True
    added_nodes = []
    for nodename, clist in list(changes.items()):
