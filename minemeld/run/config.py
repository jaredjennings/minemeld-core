...
class MineMeldConfig(_Config):
    def as_nset(self):
        result = set()
        for nname, nvalue in list(self.nodes.items()):
            result.add(
                json.dumps(
    ...
    def compute_changes(self, oconfig):
        if oconfig is None:
            ...
            for nodename, nodeattrs in list(self.nodes.items()):
                self.changes.append(
...
def resolve_prototypes(config):
    ...
    for epname, mmep in list(prototypes_entrypoints.items()):
    ...
    for _, nconfig in list(nodes_config.items()):
